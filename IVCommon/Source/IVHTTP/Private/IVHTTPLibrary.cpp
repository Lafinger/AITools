...
EHttpVerb UIVHTTPLibrary::StringToVerb(const FString& Verb)
{
...
void UIVHTTPLibrary::HttpGlobal_GetAllowedDomains(TArray<FString>& AllowedDomains)
{
	AllowedDomains = FHttpModule::Get().GetAllowedDomains();			
}

int32 UIVHTTPLibrary::HttpGlobal_GetHttpMaxConnectionsPerServer()
{
	return FHttpModule::Get().GetHttpMaxConnectionsPerServer();	   
}

FString UIVHTTPLibrary::HttpGlobal_GetProxyAddress()
{
	return FHttpModule::Get().GetProxyAddress();			
}

int32 UIVHTTPLibrary::HttpGlobal_GetMaxReadBufferSize()
{
	return FHttpModule::Get().GetMaxReadBufferSize();
}

void UIVHTTPLibrary::HttpGlobal_AddDefaultHeader(const FString& HeaderName, const FString& HeaderValue)
{
	FHttpModule::Get().AddDefaultHeader(HeaderName, HeaderValue);
}

void UIVHTTPLibrary::HttpGlobal_SetProxyAddress(const FString& InProxyAddress)
{
	FHttpModule::Get().SetProxyAddress(InProxyAddress);
}

void UIVHTTPLibrary::HttpGlobal_SetMaxReadBufferSize(const int32 SizeInBytes)
{
	FHttpModule::Get().SetMaxReadBufferSize(SizeInBytes);
}

void UIVHTTPLibrary::HttpGlobal_SetHttpTimeout(const float Timeout)
{
	FHttpModule::Get().SetHttpTimeout(Timeout);
}

void UIVHTTPLibrary::HttpGlobal_SetHttpThreadIdleMinimumSleepTimeInSeconds(const float Time)
{
	FHttpModule::Get().SetHttpThreadIdleMinimumSleepTimeInSeconds(Time);
}

void UIVHTTPLibrary::HttpGlobal_SetHttpThreadIdleFrameTimeInSeconds(const float Time)
{
	FHttpModule::Get().SetHttpThreadIdleFrameTimeInSeconds(Time);
}

void UIVHTTPLibrary::HttpGlobal_SetHttpThreadActiveMinimumSleepTimeInSeconds(const float Time)
{
	FHttpModule::Get().SetHttpThreadActiveMinimumSleepTimeInSeconds(Time);
}

void UIVHTTPLibrary::HttpGlobal_SetHttpThreadActiveFrameTimeInSeconds(const float Time)
{
	FHttpModule::Get().SetHttpThreadActiveFrameTimeInSeconds(Time);
}

void UIVHTTPLibrary::HttpGlobal_SetHttpDelayTime(const float Delay)
{
...
FString UIVHTTPLibrary::HttpResponseCodeToString(const int32 ResponseCode)
{
...
FString UIVHTTPLibrary::HttpResponseCodeToDescriptionString(const int32 ResponseCode)
{
#if WITH_EDITOR
	const UEnum* const HttpResponseCodeEnum = GetEHttpResponseCodeEnumPointer();
...
FString UIVHTTPLibrary::CreateMimeType(const EHttpMimeType Type)
{
...
EHttpResponseCode UIVHTTPLibrary::HttpResponseCodeToEnum(const int32 ResponseCode)
{
...
UIVHTTPRequset* UIVHTTPLibrary::CreateInitializedRequest(const FString& Url,
...
{
	UIVHTTPRequset* const Request = NewObject<UIVHTTPRequset>();
...
UIVHTTPRequset* UIVHTTPLibrary::CreateInitializedBinaryRequest(const FString& Url,
	const TMap<FString, FString>& UrlParameters, const EHttpVerb Verb, const EHttpMimeType MimeType,
...
FString UIVHTTPLibrary::AddParametersToUrl(FString InUrl, const TMap<FString, FString>& Parameters)
{
...
FString UIVHTTPLibrary::UrlEncodeString(const FString& StringToEscape)
{
	return FPlatformHttp::UrlEncode(StringToEscape);
}

FString UIVHTTPLibrary::UrlDecodeString(const FString& StringToDecode)
{
	return FPlatformHttp::UrlDecode(StringToDecode);
}

bool UIVHTTPLibrary::IsUrlEncoded(const TArray<uint8> Payload)
{
	return FPlatformHttp::IsURLEncoded(Payload);
}

FString UIVHTTPLibrary::GetUrlDomain(const FString& Url)
{
	return FPlatformHttp::GetUrlDomain(Url);
}

FString UIVHTTPLibrary::HtmlEncodeString(const FString& StringToEncode)
{
	return FPlatformHttp::HtmlEncode(StringToEncode);
}

FString UIVHTTPLibrary::GetOperatingSystemProxyAddress()
{
	return FPlatformHttp::GetOperatingSystemProxyAddress().Get(TEXT(""));
}

FString UIVHTTPLibrary::GetDefaultUserAgent()
{
	return FPlatformHttp::GetDefaultUserAgent();
}

FString UIVHTTPLibrary::EscapeUserAgentString(const FString& RawUserAgent)
{
	return FPlatformHttp::EscapeUserAgentString(RawUserAgent);
}

FString UIVHTTPLibrary::GetFileMimeType(const FString& FilePath)
{
