...
FIVDataTableImporterJSON::FIVDataTableImporterJSON(URunTimeTable& InDataTable, FString InJsonData)
	: DataTable(&InDataTable)
	, JSONData(MoveTemp(InJsonData))
{
}

FIVDataTableImporterJSON::~FIVDataTableImporterJSON()
{
}

bool FIVDataTableImporterJSON::ReadTable(bool bClearData)
{
	if (JSONData.IsEmpty())
	{
	...
	TArray< TSharedPtr<FJsonValue> > ParsedTableRows;
	{
		...
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Failed to parse the JSON data. Error: %s"), *JsonReader->GetErrorMessage());
			return false;
		}
	}
	...
	for (int32 RowIdx = 0; RowIdx < ParsedTableRows.Num(); ++RowIdx)
	{
		...
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Row '%d' is not a valid JSON object."), RowIdx);
			continue;
		}
...
bool FIVDataTableImporterJSON::ReadRow(const TSharedRef<FJsonObject>& InParsedTableRowObject, const int32 InRowIdx)
{
	// 获取rowname
	FString RowKey = GetKeyFieldName();
	FName RowName = DataTableUtils::MakeValidName(InParsedTableRowObject->GetStringField(RowKey));

	// 检查rowname是否为空
	if (RowName.IsNone())
	{
		UE_LOG(LogIVDataTable,Error,TEXT("Row '%d' missing key field '%s'."), InRowIdx, *RowKey);
		return false;
	}
	
	if (DataTable->GetRowMap().Find(RowName) != nullptr)
	{
		UE_LOG(LogIVDataTable,Error,TEXT("Duplicate row name '%s'."), *RowName.ToString());
		return false;
	}
	...
	if (!DataTable->bIgnoreExtraFields)
	{
		...
		for (const TPair<FString, TSharedPtr<FJsonValue>>& ParsedPropertyKeyValuePair : InParsedTableRowObject->Values)
		{
			...
			if (!ColumnProp)
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' cannot be found in struct '%s'."), *PropName.ToString(), *RowName.ToString(), *DataTable->RowStruct->GetName());
			}
		}
	}
...
bool FIVDataTableImporterJSON::ReadStruct(const TSharedRef<FJsonObject>& InParsedObject, UScriptStruct* InStruct,
	const FName InRowName, void* InStructData)
{
	...
	for (TFieldIterator<FProperty> It(InStruct); It; ++It)
	{
		...
		if (!ParsedPropertyValue.IsValid())
		{
			...
			{
					UE_LOG(LogIVDataTable,Error,TEXT("Row '%s' is missing an entry for '%s'."), *InRowName.ToString(), *ColumnName);
			}
		...
		else
		{
			...
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' is the incorrect type. Expected Array, got %s."), *ColumnName, *InRowName.ToString(), ParsedPropertyType);
				return false;
			}

			if (BaseProp->ArrayDim != PropertyValuesPtr->Num())
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' is a static sized array with %d elements, but we have %d values to import"), *ColumnName, *InRowName.ToString(), BaseProp->ArrayDim, PropertyValuesPtr->Num());
			}
...
bool FIVDataTableImporterJSON::ReadStructEntry(const TSharedRef<FJsonValue>& InParsedPropertyValue,
	const FName InRowName, const FString& InColumnName, const void* InRowData, FProperty* InProperty,
	void* InPropertyData)
{
	...
	if (FEnumProperty* EnumProp = CastField<FEnumProperty>(InProperty))
	{
		...
		if (InParsedPropertyValue->TryGetString(EnumValue))
		{
			...
			if (!Error.IsEmpty())
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' has invalid enum value: %s."), *InColumnName, *InRowName.ToString(), *EnumValue);
				return false;
			}
		}
		else
		{
			...
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' is the incorrect type. Expected Integer, got %s."), *InColumnName, *InRowName.ToString(), ParsedPropertyType);
				return false;
			}
	...
	else if (FNumericProperty *NumProp = CastField<FNumericProperty>(InProperty))
	{
		...
		else
		{
			...
			if (!InParsedPropertyValue->TryGetNumber(PropertyValue))
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' is the incorrect type. Expected Double, got %s."), *InColumnName, *InRowName.ToString(), ParsedPropertyType);
				return false;
			}
	...
	else if (FBoolProperty* BoolProp = CastField<FBoolProperty>(InProperty))
	{
		...
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' is the incorrect type. Expected Boolean, got %s."), *InColumnName, *InRowName.ToString(), ParsedPropertyType);
			return false;
		}
	...
	else if (FArrayProperty* ArrayProp = CastField<FArrayProperty>(InProperty))
	{
		...
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' is the incorrect type. Expected Array, got %s."), *InColumnName, *InRowName.ToString(), ParsedPropertyType);
			return false;
		}
	...
	else if (FMapProperty* MapProp = CastField<FMapProperty>(InProperty))
	{
		...
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' is the incorrect type. Expected Object, got %s."), *InColumnName, *InRowName.ToString(), ParsedPropertyType);
			return false;
		}
		...
		for (const auto& PropertyValuePair : (*PropertyValue)->Values)
		{
			...
			if (KeyError.Len() > 0)
			{
				...
				UE_LOG(LogIVDataTable,Error,TEXT("Problem assigning key '%s' to property '%s' on row '%s' : %s"), *PropertyValuePair.Key, *InColumnName, *InRowName.ToString(), *KeyError);
				return false;
			}
	...
	else if (FStructProperty* StructProp = CastField<FStructProperty>(InProperty))
	{
		...
		else
		{
			...
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Property '%s' on row '%s' is the incorrect type. Expected String, got %s."), *InColumnName, *InRowName.ToString(), ParsedPropertyType);
				return false;
			}
			...
			if (Error.Len() > 0)
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Problem assigning string '%s' to property '%s' on row '%s' : %s"), *PropertyValueString, *InColumnName, *InRowName.ToString(), *Error);
				return false;
			}
	...
	else
	{
		...
		if(Error.Len() > 0)
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Problem assigning string '%s' to property '%s' on row '%s' : %s"), *PropertyValue, *InColumnName, *InRowName.ToString(), *Error);
			return false;
		}
	}
...
bool FIVDataTableImporterJSON::ReadContainerEntry(const TSharedRef<FJsonValue>& InParsedPropertyValue,
	const FName InRowName, const FString& InColumnName, const int32 InArrayEntryIndex, FProperty* InProperty,
	void* InPropertyData)
{
	const TCHAR* const ParsedPropertyType = JSONTypeToString(InParsedPropertyValue->Type);
	...
	if (FEnumProperty* EnumProp = CastField<FEnumProperty>(InProperty))
	{
		...
		if (InParsedPropertyValue->TryGetString(EnumValue))
		{
			...
			if (!Error.IsEmpty())
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Entry %d on property '%s' on row '%s' has invalid enum value: %s."), InArrayEntryIndex, *InColumnName, *InRowName.ToString(), *EnumValue);
				return false;
			}
		}
		else
		{
			...
			if (!InParsedPropertyValue->TryGetNumber(PropertyValue))
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Entry %d on property '%s' on row '%s' is the incorrect type. Expected Integer, got %s."), InArrayEntryIndex, *InColumnName, *InRowName.ToString(), ParsedPropertyType);
				return false;
			}
	...
	else if (FNumericProperty *NumProp = CastField<FNumericProperty>(InProperty))
	{
		...
		else
		{
			...
			if (!InParsedPropertyValue->TryGetNumber(PropertyValue))
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Entry %d on property '%s' on row '%s' is the incorrect type. Expected Double, got %s."), InArrayEntryIndex, *InColumnName, *InRowName.ToString(), ParsedPropertyType);
				return false;
			}
	...
	else if (FBoolProperty* BoolProp = CastField<FBoolProperty>(InProperty))
	{
		...
		if (!InParsedPropertyValue->TryGetBool(PropertyValue))
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Entry %d on property '%s' on row '%s' is the incorrect type. Expected Boolean, got %s."), InArrayEntryIndex, *InColumnName, *InRowName.ToString(), ParsedPropertyType);
			return false;
		}
	...
	else if (FStructProperty* StructProp = CastField<FStructProperty>(InProperty))
	{
		...
		else
		{
			...
			if (Error.Len() > 0)
			{
				UE_LOG(LogIVDataTable,Error,TEXT("Problem assigning string '%s' to entry %d on property '%s' on row '%s' : %s") ,*PropertyValueString, InArrayEntryIndex,*InColumnName, *InRowName.ToString(), *Error);
				return false;
			}
	...
	else
	{
		...
		if (!InParsedPropertyValue->TryGetString(PropertyValue))
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Entry %d on property '%s' on row '%s' is the incorrect type. Expected String, got %s."), InArrayEntryIndex, *InColumnName, *InRowName.ToString(), ParsedPropertyType);
			return false;
		}
		...
		if(Error.Len() > 0)
		{
			UE_LOG(LogIVDataTable,Error,TEXT("Problem assigning string '%s' to entry %d on property '%s' on row '%s' : %s"), *PropertyValue,InArrayEntryIndex, *InColumnName, *InRowName.ToString(), *Error);
			return false;
		}
	}
...
FString FIVDataTableImporterJSON::GetKeyFieldName()
{
